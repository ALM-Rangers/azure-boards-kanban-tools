...
const BaseWiql = "SELECT [System.Id],[System.WorkItemType],[System.Title] FROM workitems WHERE [System.TeamProject] = @project " +
...
const WiqlWorkItemTypes = "@WorkItemTypes";
const WiqlBoardColumns = "@OldBoardColumns";
const WiqlRootArea = "@RootArea";
const WiqlIteration = "@RootIteration";
const WiqlWorkItemColumnField = "@WITField";
const DefaultRowId = "00000000-0000-0000-0000-000000000000";

export function getTeamsAsync(excludeCurrentTeam?: boolean): Promise<CoreContracts.WebApiTeam[]> {
...
export function getTeam(teamName?: string): Promise<CoreContracts.WebApiTeam> {
    return new Promise<CoreContracts.WebApiTeam>((resolve, reject) => {
        const webContext = VSS.getWebContext();
        const client = CoreRestClient.getClient();
...
export function getCommonBacklogLevels(team1Settings: Models.IBoardSettings, team2Settings: Models.IBoardSettings): string[] {
...
export async function getTeamSettingsAsync(context: CoreContracts.TeamContext): Promise<Models.IBoardSettings> {
...
export async function applyTeamSettingsAsync(oldSettings: Models.IBoardSettings, settingsToApply: Models.IBoardSettings, selectedMappings: Models.IBoardColumnDifferences[], selectedBacklogLevels: string[]): Promise<Boolean> {
    ...
    try {
        for (let backlogIndex = 0; backlogIndex < settingsToApply.backlogSettings.length; backlogIndex++) {
            ...
            wiql.query = wiql.query.replace(WiqlWorkItemTypes, workItemTypes);
            wiql.query = wiql.query.replace(WiqlIteration, teamSettings.backlogIteration.name);
            wiql.query = wiql.query.replace(WiqlRootArea, areaQuery);
            wiql.query = wiql.query.replace(WiqlBoardColumns, wiqlColumns);
            wiql.query = wiql.query.replace(WiqlWorkItemColumnField, boardColumnField);
...
async function _applyNewRows(outgoingRows: WorkContracts.BoardRow[], incomingRows: WorkContracts.BoardRow[], uniqueNameifier: string, boardName: string, context: CoreContracts.TeamContext, client: WorkClient.WorkHttpClient2_3): Promise<WorkContracts.BoardRow[]> {
...
async function _cleanUpRows(currentRows: WorkContracts.BoardRow[], incomingRows: WorkContracts.BoardRow[], uniqueNameifier: string, boardName: string, context: CoreContracts.TeamContext, client: WorkClient.WorkHttpClient2_3): Promise<WorkContracts.BoardRow[]> {
...
function _getDefaultRow(rows: WorkContracts.BoardRow[]): WorkContracts.BoardRow {
    let defaultRows = rows.filter(row => row.id === DefaultRowId);
...
function _compareColumnStateMappings(c1: WorkContracts.BoardColumn, c2: WorkContracts.BoardColumn): boolean {
